  ...
  if ('serviceWorker' in navigator) {
    ...
    window.addEventListener('load', async () => {
      ...
      try {
        ...
        if (serviceWorker.active) console.log('Service worker registration successful!');
      } catch (err) {
        // B5. In the event that the service worker registration fails, console
        //     log that it has failed.
        console.log('Server worker registration failed');
      }
  });
  } else {
...
async function getRecipes() {
  ...
  if (existingRecipes) return JSON.parse(existingRecipes);
  /**************************/
